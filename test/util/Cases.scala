...
object Cases {
  ...
  def withoutAgent(): Case => Case = { c =>
    c.copy(application = c.application.asInstanceOf[BTIApplication].copy(agent = None))
  }
  ...
  def withContact(contact: Contact): Case => Case = { c =>
    c.copy(application = c.application.asInstanceOf[BTIApplication].copy(contact = contact))
  }
