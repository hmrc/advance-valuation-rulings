...
class ValuationCaseRepository @Inject() (mongo: MongoComponent)(implicit ec: ExecutionContext)
  ...
  def assignCase(reference: String, caseWorker: CaseWorker): Future[Long] =
    for {
      result <- collection
                  .updateOne(
                    Filters.equal("reference", reference),
                    Updates.combine(
                      Updates.set("status", CaseStatus.REFERRED.toString),
                      Updates.set("assignee", Codecs.toBson(caseWorker))
                    )
                  )
                  .toFuture()
    } yield
      if (result.wasAcknowledged()) result.getModifiedCount
      else throw new Exception("failed to assign case")
      ...
      result <-
        collection
          .updateOne(
            Filters.equal("reference", reference),
            Updates
              .combine(Updates.set("status", CaseStatus.OPEN.toString), Updates.unset("assignee"))
          )
          .toFuture()
    } yield
      if (result.wasAcknowledged()) result.getModifiedCount
      else throw new Exception("failed to unassign case")

  def create(valuation: ValuationCase): Future[BsonObjectId] =
    collection.insertOne(valuation).toFuture().map {
      result =>
        if (result.wasAcknowledged()) result.getInsertedId.asObjectId()
  ...
  def allNewCases(): Future[List[ValuationCase]] =
    collection.find[ValuationCase](Filters.equal("status", "NEW")).toFuture().map(_.toList)
  ...
  def replaceItem(valuationCase: ValuationCase): Future[Long] =
    collection
      .replaceOne(Filters.equal("reference", valuationCase.reference), valuationCase)
      .toFuture()
      .map {
        result =>
          if (result.wasAcknowledged()) result.getModifiedCount
