...
class InternalAuthInitialiserSpec extends BaseIntegrationSpec with WireMockHelper {
  ...
  def expectedRequest(appName: String, authToken: String): JsObject = Json.obj(
    "token"       -> authToken,
    "principal"   -> appName,
    "permissions" -> Seq(
      Json.obj(
  ...
  "when configured to run" should {
    ...
    "must initialise the internal-auth token if it is not already initialised" in {
      ...
      wireMockServer.verify(
        ...
        postRequestedFor(urlMatching("/test-only/token"))
          .withRequestBody(equalToJson(Json.stringify(Json.toJson(expectedRequest(appName, authToken)))))
      )
    }
    ...
    "must not initialise the internal-auth token if it is already initialised" in {
      ...
      app.injector
        .instanceOf[InternalAuthTokenInitialiser]
        .initialised
        .futureValue
